...
class RRAudioEngine: NSObject {
    
    static let instance: RRAudioEngine = RRAudioEngine()
    ...
    var currentAbility: RRAudioEngineAbility = .undefined
    
    var status: RRAudioEngineStatus = .unReady
    
    var useCase: RRAudioEngineUseCase = .streamingWithHighQuality
    
    weak var dataSource: RRAudioEngineDataSource?
    
    weak var delegate: RRAudioEngineDelegate?
    ...
    var maxDistanceWithListener: RRDistanceMeters = -1
    
    var staticDistanceWithListener: RRDistanceMeters?
    ...
    func setupAudioEngineEnvironmentForScheduleAudioData() {
        ...
        case .undefined, .CaptureAudioData:
            let taskAfterSetupInputAudioUnit = {[weak self] in
                ...
                self.setupRendererAudioData(updatedInterval: RRAudioEngine.metaDataUpdatedInterval)
    ...
    func setStaticDistanceWithListener(_ distance: RRDistanceMeters?) {
    ...
    private func setupOutputNodeAttachment() {
        ...
        outputEngine.attach(streamingMixer)   // Console: RoundRecorder[56842:741656] throwing -10878
    ...
    private func setupRendererAudioUnit(_ completion: @escaping()->Void ) {
        ...
        RRAudioRenderAudioUnit.registerSubclassOnce
        
        AVAudioUnit.instantiate(with: RRAudioRenderAudioUnit.audioCompoentDescription, options: []) { auUnit, error in
    ...
    private func setupRendererAudioData(updatedInterval: Double) {
        rendererData = RRAudioOutputData(format: convertFormat, dataMS: rendererDataMilliseconds, bufferMS: rendererDataBufferMilliseconds)
    ...
    public func schechuleRendererAudioBuffer(_ buffer: RRAudioBuffer) {
        // ScheduleAudioBuffer
        rendererData?.scheduleRRAudioBuffer(buffer: buffer)
    }
    
    private func captureAudioInputBuffer(_ audioBuffer: AudioBuffer) {
        guard let currentLocation = dataSource?.rrAudioEngine(currentLocationForEngine: self) else { return }
        let trueNorthAnchorsMotion = dataSource?.rrAudioEngine(currentTrueNorthAnchorsMotionForEngine: self)
        ...
        let rrAudioData = RRAudioEngine.encodeRRAudioBufferData(date, bufferLenght, nChannel, sampleRate, bitRate, latitude, longitude, altitude, roll, pitch, yaw, audioData)
        
        delegate?.captureAudioBufferDataCallBack(self, rrAudioData: rrAudioData)
    }
    // MARK: Player feature
    public func setupPlayerDataAndStartPlayingAtSeconds(_ data: RRAudioData, startOffset: Second, updateInterval: Second, updateDurationCallback: @escaping ((Second)->Void), endOfFilePlayingCallback: @escaping ((Second)->Void)) {
        // 1. Generate URAudioOutputData with data formatt
        playerData = RRAudioOutputData(data: data)
    ...
    private func updateReceivingMetadata(_ receivingMetadata: RRAudioBufferMetadata) {
        // View
        delegate?.didUpdateReceiversBufferMetaData(self, metaData: receivingMetadata)
        
        // MARK: Update Orientation
        if let userTrueNorthAnchorsMotion = self.dataSource?.rrAudioEngine(currentTrueNorthAnchorsMotionForEngine: self) {
        ...
        if let userLocation = self.dataSource?.rrAudioEngine(currentLocationForEngine: self) {
            ...
            // Audio Engine
            let listenerPosition = RRAudioEngine.get3DMetersPositionWith(directionAndDistance, staticDistanceWithListener: staticDistanceWithListener)
     ...
     */
    // MARK: - Parse data
    static func parseRRAudioBufferData(_ data: Data)->(RRAudioBuffer) {
        ...
        let location = RRLocationCoordinate3D(latitude: latitude,
                                              longitude: longitude,
                                              altitude: altitude)
        
        let trueNorthMotion = RRMotionAttitude(rollDegrees: trueNorthRollDegrees,
                                      pitchDegrees: trueNorthPitchDegrees,
                                      yawDegrees: trueNorthYawDegrees)
        
        let metadata: RRAudioBufferMetadata = RRAudioBufferMetadata(locationCoordinate: location,
                                                                    motionAttitude: trueNorthMotion)
        
        let buffer = RRAudioBuffer(mData, audioBufferLength, channel, sampleRate, bitRate, metadata, date)
        
        return buffer
    }
    
    static func parseRRAudioBufferData(_ data: Data, audioBuffersSize: Int)->[RRAudioBuffer] {
        var readingOffset: Int = 0
        
        var bufferCollectinos: [RRAudioBuffer] = []
    ...
    static func encodeRRAudioBufferData(_ date: UInt64,
...
extension RRAudioEngine {
    private func generateCategoryOptions(_ useCase: RRAudioEngineUseCase, supportBluetoothA2Dp: Bool)-> AVAudioSession.CategoryOptions {
...
extension RRAudioEngine: RRAudioRenderAudioUnitDelegate {
...
extension RRAudioEngine {
    static func get3DMetersPositionWith(_ directionAndDistance: RR3DDirectionAndDistance, staticDistanceWithListener: RRDistanceMeters?) -> AVAudio3DPoint {
        ...
        let maxDistance: RRDistanceMeters = 5
